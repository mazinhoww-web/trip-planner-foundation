import { ReactNode } from 'react';
import { Button } from '@/components/ui/button';
import { RefreshCcw, Users } from 'lucide-react';

type TripTopActionsProps = {
  isReconciling: boolean;
  onReconcile: () => void;
  showManageUsers?: boolean;
  onManageUsers?: () => void;
  children?: ReactNode;
};

export function TripTopActions({
  isReconciling,
  onReconcile,
  showManageUsers = false,
  onManageUsers,
  children,
}: TripTopActionsProps) {
  return (
    <div className="mb-6 rounded-2xl border border-primary/15 bg-white/90 p-3 shadow-sm sm:p-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
          <Button
            variant="outline"
            onClick={onReconcile}
            disabled={isReconciling}
            aria-label="Reconciliar dados com banco"
            className="w-full border-primary/25 text-primary hover:bg-primary/5 sm:w-auto"
          >
            <RefreshCcw className={`mr-2 h-4 w-4 ${isReconciling ? 'animate-spin' : ''}`} />
            Reconciliar dados
          </Button>
          {showManageUsers && onManageUsers ? (
            <Button
              variant="outline"
              onClick={onManageUsers}
              aria-label="Ir para usuários e convites"
              className="w-full border-primary/25 text-primary hover:bg-primary/5 sm:w-auto"
            >
              <Users className="mr-2 h-4 w-4" />
              Usuários e convites
            </Button>
          ) : null}
        </div>
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:justify-end">
          {children}
        </div>
      </div>
    </div>
  );
}
