...
// ─── Itinerary multi-item extraction ──────────────────────────────

export type ItineraryItemType = 'voo' | 'hospedagem' | 'transporte' | 'restaurante' | 'atividade';
...
export type ExtractedItinerary = {
  items: ExtractedItineraryItem[];
  resumo_viagem: {
    destino: string | null;
    data_inicio: string | null;
    data_fim: string | null;
...
export async function extractItineraryStructured(text: string, fileName: string): Promise<ExtractedItinerary> {
  const { data, error } = await supabase.functions.invoke('extract-itinerary', {
    body: { text, fileName },
  });

  if (error) throw new Error(parseFunctionError(data ?? error, 'Falha na extração do roteiro.'));
  if (data?.error) throw new Error(parseFunctionError(data, 'Falha na extração do roteiro.'));

  const parsed = data?.data;
  if (!parsed || !Array.isArray(parsed.items)) throw new Error('Extração do roteiro retornou vazio.');

  return {
    items: parsed.items.map((item: any) => ({
      tipo: item.tipo,
      confianca: item.confianca ?? 50,
      dados: item.dados ?? {},
      selected: true,
    })),
    resumo_viagem: parsed.resumo_viagem ?? { destino: null, data_inicio: null, data_fim: null },
  };
}
